...
import { Contribution, FetchType } from "contributions-getter";
...
${PRIMARY_LANGUAGE_SYMBOL}\n${REPO_DESCRIPTION_SYMBOL}`;
const HIGHLIGHT_FORMAT = `[COOL] ${HEADER_SYMBOL} [COOL]`;
const FILE_BEFORE = "./before.md";
const FILE_AFTER = "./after.md";
...
describe("Full path till returning the markdown", () => {
  it("returns correct markdown with first combination", async () => {
    ...
    expect(markdown).toBe(FIRST_EXPECTED_MARKDOWN);
  });
});
